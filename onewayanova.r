...
dataoneway$Group = factor(dataoneway$Group,labels = c("Quizz", "Coding", "Videos"))
...
Group1 <- subset(dataoneway, Group == "Quizz")
Group2 <- subset(dataoneway, Group == "Coding")
Group3 <- subset(dataoneway, Group == "Videos")
...
ggplot(dataoneway, aes(x = Group, y = Length)) +
  ...
  scale_x_discrete() + xlab("Learning type") +
  ylab("Time (minutes)")
